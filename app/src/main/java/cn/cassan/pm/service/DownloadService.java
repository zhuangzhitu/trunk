            ...
            switch (msg.what) {
                ...
                case 1:
                    ...
                    if (rate < 100) {
                        ...
                        contentview.setTextViewText(cn.cassan.pm.R.id.tv_download_state, mTitle + "(" + rate
                                + "%" + ")");
                        contentview.setProgressBar(cn.cassan.pm.R.id.pb_download, 100, rate,
                                false);
                    } else {
                        // 下载完毕后变换通知形式
...
//							"文件已下载完毕", contentIntent);
    ...
    private void setUpNotification() {
        int icon = cn.cassan.pm.R.drawable.ic_notification;
        CharSequence tickerText = "准备下载";
        long when = System.currentTimeMillis();
        mNotification = new Notification(icon, tickerText, when);
        // 放置在"正在运行"栏目中
        mNotification.flags = Notification.FLAG_ONGOING_EVENT;
        RemoteViews contentView = new RemoteViews(getPackageName(), cn.cassan.pm.R.layout.download_notification_show);
        contentView.setTextViewText(cn.cassan.pm.R.id.tv_download_state, mTitle);
        // 指定个性化视图
