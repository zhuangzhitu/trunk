package com.cassan.circlefriend.widgets;
...
import com.cassan.circlefriend.spannable.CircleMovementMethod;
import com.cassan.circlefriend.spannable.SpannableClickable;
...
public class PraiseListView extends TextView {
    ...
    public void notifyDataSetChanged() {
        ...
        if (datas != null && datas.size() > 0) {
            ...
            for (int i = 0; i < datas.size(); i++) {
                item = datas.get(i);
                if (item != null) {
                    builder.append(setClickableSpan(item.getUser().getUsername(), i));
                    if (i != datas.size() - 1) {
                        builder.append(", ");
                    }
                }
    ...
    private SpannableString setImageSpan() {
        ...
        imgSpanText.setSpan(new ImageSpan(AppContext.getInstance(), R.drawable.icon_praise, DynamicDrawableSpan.ALIGN_BASELINE),
