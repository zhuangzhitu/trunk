package com.cassan.circlefriend.utils;
...
public class DatasUtil {
    ...
    public static final UserInfo curUser = AppContext.getInstance().getLoginUserInfo();
    //	static {
     ...
     * @return
     */
    public static CommentItem createReplyComment(UserInfo replyUser, String content) {
        CommentItem item = new CommentItem();
        item.setId(String.valueOf(commentId++));
        item.setContent(content);
        item.setUser(curUser);
    ...
    //	}
    //请求数据
    public static ArrayList<CircleItem> datas;
    ...
    public static void handleWorkBenchNoticeBean(ApiReturnResult apiReturnResult, Header[] headers) {

        if (apiReturnResult.isOK()) {
            String data = apiReturnResult.getData();
            Gson gson = new Gson();
            CircleItem[] topNews = gson.fromJson(data, CircleItem[].class);
            List<CircleItem> topNewList = Arrays.asList(topNews);
            // 不这样做的话会addAll报错
            datas = new ArrayList<CircleItem>(topNewList);
        } else {
            datas = null;
        }
